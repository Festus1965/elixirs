...
local function throw_cocktail(item, player)
	...
	local obj = minetest.add_entity({x=playerpos.x,y=playerpos.y+1.625,z=playerpos.z}, 'elixirs:molotov_entity')
	...
	if self.timer>0.2 then
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= 'elixirs:molotov_entity' and obj:get_luaentity().name ~= '__builtin:item' then
					if self.node ~= '' then
...
minetest.register_entity('elixirs:molotov_entity', {
	timer=0,
...
minetest.register_craftitem('elixirs:molotov_cocktail', {
...
minetest.register_craft({
	output = "elixirs:molotov_cocktail",
  type = 'shapeless',
	recipe = {
		"farming:cotton", "elixirs:naptha",
	},
})
